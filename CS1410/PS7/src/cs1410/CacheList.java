...
public class CacheList
{
    ...
    private double difficultyMinConstraint;
    private double difficultyMaxConstraint;
    private double terrainMinConstraint;
    private double terrainMaxConstraint;
    ...
    public CacheList (Scanner caches) throws IOException
    {
        titleConstraint = "";
        ownerConstraint = "";
        difficultyMinConstraint = 1.0;
        difficultyMaxConstraint = 5.0;
        terrainMinConstraint = 1.0;
        terrainMaxConstraint = 5.0;
        // TODO: Complete this implementation
        allCaches = new ArrayList<Cache>();
        
      //  String allCachesArray[] = caches.split("\n");
        ...
        try
        {
            while (caches.hasNextLine())
            {
                ...
                allCaches.add(new Cache(attributes));
    ...
    {
        difficultyMinConstraint = min;
        difficultyMaxConstraint = max;
    }
    ...
    {
        terrainMinConstraint = min;
        terrainMaxConstraint = max;
    }
    ...
    public ArrayList<Cache> select ()
    {
        ...
        {
            if(allCaches.get(i).getTitle().toLowerCase().contains(titleConstraint) && allCaches.get(i).getOwner().toLowerCase().contains(ownerConstraint)
                    && allCaches.get(i).getDifficulty() >= difficultyMinConstraint && allCaches.get(i).getDifficulty() <= difficultyMaxConstraint
                    && allCaches.get(i).getTerrain() >= terrainMinConstraint && allCaches.get(i).getTerrain() <= terrainMaxConstraint)
            {
                caches.add(allCaches.get(i));
            }
        }
